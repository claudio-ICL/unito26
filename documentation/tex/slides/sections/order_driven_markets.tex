...
% Budget 105 min; planned 95 over 22 frames, with the welcome frames ahead of them.
...
Today: orders, matching, the book at an instant, what trading costs.

\pause

1.4 is what the rest is for.
\note{
...
What the next frame's \emph{total} order needs.
\note{
...
\alert{$\rho \wedge q$ shares at $\pi$ --- never at $p$.}

\pause

\emph{Active} at $u$: submitted, not executed, not cancelled.
\note{
...
A limit order book always means this \emph{aggregated} book.
...
$\midPrice_t$ is what we track for the rest of the chapter.
\note{
...
Bid-heavy is positive. \alert{Inverting this sign inverts every signal built on it.}
...
A second route to the same number, and the notebook takes both.
\note{
...
The second term is a grid distance and not a count of queues, which the next frame is the
picture of.
}
\end{frame}
...
\alert{One code path: consume, then rest the remainder.}
\note{
