...
\lbrace (\nthBestAskVolume[i]_t, \nthBestBidVolume[i]_t) : i=1,2,\dots \rbrace \Big),
\]
...
and the $n$-levels volume imbalance
\[
...
The queue imbalance is widely accepted as a reliable signal for the next mid-price move
\citep{CDJ18enh}:
...
Executing a signed size $\volume$ at time $t$ by \emph{market order}:
\[
