...
and \eqref{eq.scalarVariance} understates $\mathbf{1}^{\transpose}V\mathbf{1}$ by $4.9\%$
--- a discrepancy that does not depend on $\decay$, since both sides scale as $1/\decay$.
Four per cent is small, and quoting the scalar formula for a quantity it does not describe is
the kind of error that stays small until it does not.
