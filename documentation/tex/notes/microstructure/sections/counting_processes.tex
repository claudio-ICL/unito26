...
\begin{remark}\label{remark.internalHistory}
 Notice that the internal history is a hypothesis of
 Proposition \ref{prop.conditionalSurvival} and not a convenience.
 On a strictly larger history the intensity need not be a function of the past of
 $\multiCountingProc$ alone --- on a Cox process it is not ---
 the integrand in \eqref{eq.conditionalSurvival} is then not
 $\filtrationF_{\arrivalTimes[ ]_{n}}$-measurable,
 and what survives is the conditional expectation of the right-hand side rather than the
 right-hand side itself.
\end{remark}

Compensators are the ingredient of the following time-change result,
which we shall use in goodness-of-fit diagnostics.
