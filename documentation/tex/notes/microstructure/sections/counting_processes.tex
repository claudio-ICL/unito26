...
and let
\begin{equation}
 \label{eq.arrivalTimesOfGroundProcess}
...
Define the $n$-th event label
\begin{equation}
 \label{eq.eventLabels}
 \event := \sum_{e=1}^{\numEventTypes} e \,
 \indicator{\delta \groundProc(\arrivalTimes[ ]_{n}) = \delta\countingProc(\arrivalTimes[ ]_{n})},
 \qquad n = 1, 2, \dots
\end{equation}
The pair $(\arrivalTimes[ ]_{n}, \event[n])$ characterises $\multiCountingProc$ equivalently,
because
...
\begin{example}[``Poisson process'']
 ...
 Since $G_{e,j}(0) = \Prob (\countingProc(0)\geq j)$,
...
Both simulation algorithms of Section \ref{sec.simulation} rest on the following
consequence, which we therefore record separately.
...
\begin{prop}\label{prop.conditionalSurvival}
 Let $\multiCountingProc$ admit an intensity $\intensity[ ]$ with respect to its own
 internal history $\filtrationF$,
...
For a proof, see \cite[Chapter II]{Bre81poi}.
On the event that no arrival occurs, the integrand is a deterministic function of the history
up to $\arrivalTimes[ ]_{n}$,
so the right-hand side is $\filtrationF_{\arrivalTimes[ ]_{n}}$-measurable
and \eqref{eq.conditionalSurvival} is a genuine survival function, which we can invert.
For a homogeneous Poisson process it reduces to $e^{-\totalIntensity s}$,
and the inter-arrival times are exponential.

\begin{remark}\label{remark.internalHistory}
 Notice that $\filtrationF$ is the \emph{internal} history.
 On a larger one the intensity need not be a function of the past of $\multiCountingProc$
 alone --- on a Cox process whose directing intensity is revealed progressively it is not ---
 and the right-hand side of \eqref{eq.conditionalSurvival} is then no longer
 $\filtrationF_{\arrivalTimes[ ]_{n}}$-measurable.
\end{remark}
...
\begin{thm}[{\cite{Mey71dem}}]\label{thm.meyer1971}
 ...
 \begin{equation}\label{eq.time-changed_intertimes}
  \tau_e^j := \compensator_e(\arrivalTimes_{j}) - \compensator_e(\arrivalTimes_{j-1})
 \end{equation}
