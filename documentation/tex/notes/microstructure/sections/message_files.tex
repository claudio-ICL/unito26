...
\cite{CMEmdp}, and Nasdaq publishes the message stream as TotalView-ITCH \cite{NAS23itc}.
...
\begin{remark}\label{remark.derivedAndDelayed}
 Notice that the aggregated book is derived:
 a participant who subscribes to it observes that cross-section only once the exchange has
 computed and published it.
 It is also truncated at depth $L$, and where the feed is netted its updates are sent at
 regular intervals, so that intermediate configurations are not merely delayed but absent.
...
Reconstruction is an ordinary part of a trading system.
