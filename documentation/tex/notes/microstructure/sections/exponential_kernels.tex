...
In an exponential-kernel Hawkes process, the branching matrix is 
\begin{equation}\label{eq.GammaIsAOverBeta}
 \branchingMatrix = \frac{\excitation}{\decay}.
\end{equation}
Observe that the branching matrix is dimensionless while the excitation matrix is a rate.
...
\begin{remark}\label{remark.readPreJump}
 ...
 $\timeHorizon \mapsto \countingProc[\eone](\timeHorizon -)$
 and $\timeHorizon \mapsto \decayedCounts_{\eone}(\timeHorizon)$
 are both left-continuous by construction.
 On the other hand, at every arrival time $\arrivalTimes[\eone]_{j}$,
 \begin{equation*}
  \countingProc[\eone](\arrivalTimes[\eone]_{j})
  - \countingProc[\eone](\arrivalTimes[\eone]_{j} - )
  = \decayedCounts_{\eone}(\arrivalTimes[\eone]_{j} + )
  - \decayedCounts_{\eone}(\arrivalTimes[\eone]_{j} ),
 \end{equation*}
 so the two jumps cancel.
 Theorem \ref{thm.meyer1971} requires that continuity.
 ...
 arrival of type $\eone$, and Section \ref{sec.simulation} evaluates the compensator at
 exactly those times.
\end{remark}

Hawkes processes with arbitrary kernels
might have complicated dependence on their history.
Exponential kernels instead have the remarkable property that
the dependence of the intensity on the history is
summarised in the decayed count.
In other words,
the decayed count at time $t$ is sufficient
to characterise the law of the process after $t$.
Between arrivals the state decays deterministically,
along the flow $\varphi_t(z) = e^{-\decay t}z$,
and at each arrival it gains a unit vector.

\begin{thm}[Markovianity of exponential-kernel Hawkes processes]\label{thm.markovState}
...
\begin{exercise}\label{exercise.eventDependentDecays}
 We work with exponential kernels that share a single $\decay$ across all components,
 so that the decayed count is $\numEventTypes$-dimensional.
 A decay $\decay\subscriptee$ per pair would require one decayed count per pair,
 a state of dimension $\numEventTypes^2$,
 and every expression of Section \ref{sec.stability} would have to be rewritten on it.
 Rewrite them.
\end{exercise}
