...
\begin{lemma}[Perron-Frobenius]\label{lemma.perronFrobenius}
 ...
 and non-negative;
 \emph{ii.} there are non-negative, non-zero vectors $w$ and $v$ with
 $\branchingMatrix w = \branchingRatio w$ and
 ...
 \emph{iii.} $\branchingRatio$ lies between the smallest and the largest row sum of
 $\branchingMatrix$;
 \emph{iv.} if $\branchingRatio < 1$ then $I - \branchingMatrix$ is invertible and
...
Under irreducibility $w$ and $v$ are strictly positive and $\branchingRatio$ is simple.
...
A stochastic process is \emph{stationary} when its law is invariant under time shift:
for every $r$ and every $t_1 < \dots < t_r$,
the joint law of $(X_{t_1+h}, \dots, X_{t_r+h})$ does not depend on $h$.
For a point process the same is asked of the counts,
the joint law of $\big(\multiCountingProc(A_1+h),\dots,\multiCountingProc(A_r+h)\big)$ being
independent of $h$ for bounded Borel sets $A_1,\dots,A_r$;
see \cite[Chapter 12]{DVJ08int} or \cite[Chapter VIII]{Bre81poi}.
Shifting moves the origin,
so stationarity is a property of a process defined on the whole line.
...
What $\branchingRatio < 1$ supplies is Lemma \ref{lemma.perronFrobenius}~\emph{iv.}:
...
\begin{remark}\label{remark.strictBaseline}
 ...
 and the left-hand side strictly positive.
 ...
 because nothing reaches the first type --- it has neither immigrants nor an exciter ---
 and the second is a Poisson process.
\end{remark}
...
\begin{prop}\label{prop.clusterSize}
 ...
 The expected size $C_{\eone}$ of the cluster founded by an immigrant of type $\eone$,
 counting the immigrant itself, is
 \begin{equation}\label{eq.clusterSize}
  C_{\eone} = \mathbf{1}^{\transpose} (I - \branchingMatrix)^{-1} \unitVector_{\eone} ,
 \end{equation}
 and the expected number of its strict descendants is $C_{\eone} - 1$.
...
Notice that $\branchingRatio$ is in general neither the endogenous fraction nor
$1-1/\bar C$.
The two agree in particular when
$\mathbf{1}^{\transpose}\branchingMatrix = \branchingRatio\mathbf{1}^{\transpose}$
and when $\branchingMatrix\baseIntensity = \branchingRatio\baseIntensity$,
neither of which is generic:
$\branchingRatio$ does not depend on $\baseIntensity$, and
\eqref{eq.meanClusterSize} does.
...
Less fails without it than one expects.
...
quantities above.
Each of these fails at $\branchingRatio \geq 1$,
provided $\baseIntensity > 0$ for the reason of Remark \ref{remark.strictBaseline}.
Contracting $\dot m_t = (\excitation - \decay I) m_t + \baseIntensity$ with a left Perron
...
\begin{equation*}
 ...
 \end{cases}
\end{equation*}
so the intensity grows exponentially at rate $\decay(\branchingRatio-1)$ above criticality,
and at $\branchingRatio = 1$ linearly, with a quadratic event count,
whenever $1$ is a semisimple eigenvalue of $\branchingMatrix$.
