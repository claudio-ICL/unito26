...
if $\marketOrderSize > \nthBestAskSize[1]_{t-}$.
A market order walks the book if and only if
its sweep cost is larger than half the bid-ask spread,
the walk term in \eqref{eq.sweepCost} being positive
exactly when some $q_i$ with $i\geq 2$ is non-zero.
