...
Recall the arrival times $\arrivalTimes[ ]_n$ of the ground process
from \eqref{eq.arrivalTimesOfGroundProcess},
and the event labels from \eqref{eq.eventLabels}.
The idea is Proposition \ref{prop.conditionalSurvival} run backwards.
If $\totalIntensity$ were constant, 
the next inter-arrival would be exponential of that rate.
It is not constant,
but if it is non-increasing between events then its value at the last event
dominates it,
...
The dominating value is the \emph{post-jump} one, $\totalIntensity(\arrivalTimes[]_{n-1}+)$:
$\decayedCounts$ is left-continuous, so the pre-jump value is smaller by
$(\mathbf{1}^{\transpose}\excitation)_{\event[n-1]}$ and does not dominate.

We take $(\arrivalTimes[]_n,\event[n])$ as the sequence of arrival times and event labels
of the ground process.
For a general kernel, computing $\totalIntensity(t)$ requires
$\lbrace(\arrivalTimes[]_n,\event[n]): \, \arrivalTimes[]_n <t\rbrace$,
the whole history up to $t$.
The algorithm is detailed in Algorithm \ref{algo.ogata}.
...
\begin{algorithm}[h!]
 ...
 \begin{algorithmic}[5]
  ...
  \STATE set $t:=\arrivalTimes[]_{n-1}$ and
         $\bar{\lambda} := \totalIntensity(\arrivalTimes[]_{n-1}+)$
  \STATE set $\xi:=0$
  \WHILE{$\xi=0$}
  \STATE draw $U \sim \text{Exp}(\bar{\lambda})$
  \STATE set $\xi :=1$ with probability $\totalIntensity(t+U)/\bar{\lambda}$
  \STATE update $t\leftarrow t+U$ and $\bar{\lambda} \leftarrow \totalIntensity(t)$
  \ENDWHILE
  \STATE set $\arrivalTimes[]_n:=t$
  \STATE draw $\event[n]$ in $\lbrace 1,\dots,\numEventTypes\rbrace$ with probabilities
         proportional to
         $\lbrace \intensity[1](\arrivalTimes[]_n), \dots,
         \intensity[\numEventTypes](\arrivalTimes[]_n)\rbrace$
  \RETURN $(\arrivalTimes[]_n,\event[n])$
 \end{algorithmic}
\end{algorithm}

Algorithm \ref{algo.ogata} evaluates $\totalIntensity$ three times:
twice in the loop, and once more to draw $\event[n]$.
For a general kernel each of the three reads the history.
For an exponential kernel each reads $\decayedCounts$ instead:
by Theorem \ref{thm.markovState} the pair $(\multiCountingProc,\decayedCounts)$ is Markov, so
$\totalIntensity(t) = \mathbf{1}^{\transpose}(\baseIntensity + \excitation\decayedCounts(t))$,
and the algorithm holds $\numEventTypes$ numbers in place of a list that grows with the path.
The state is carried from event to event by a recursion, not recomputed.
Between $\arrivalTimes[ ]_{n}$ and $\arrivalTimes[ ]_{n+1}$ it merely decays,
...
\end{equation}
The right-hand side is a product of two factors, so, as \cite{DZ13exa} observed,
the inter-arrival time of the ground process is the minimum of two independent waiting times,
one for the immigration and one for the excitation.
The waiting time of the immigration is exponential of parameter $\bar{\baseIntensity}$.
The waiting time for the excitation is \emph{defective}:
with probability $e^{-D/\decay} > 0$ no excitation occurs at all.


\begin{remark}\label{remark.commonDecay}
 Notice that \eqref{eq.totalIntensityBetweenEvents} 
	uses crucially that $\decay$ is common to all
 pairs.
 With a decay $\decay\subscriptee$ depending on the pair, or even one depending only on the
 type of the exciting event, 
 the components of $\decayedCounts$ decay at different rates and
 $\totalIntensity$ is a sum of exponentials rather than one exponential,
 so that the two-factor form of \eqref{eq.dzSurvival} fails,
 and with it Theorem \ref{thm.dassiosZhao} as stated.
 The survival function still factorises, over one clock per decay rate rather than two,
 so the scheme generalises at the cost of carrying one clock more.
\end{remark}
...
\begin{proof}
 ...
 The two are independent, so the survival function of $\tau_1 \wedge \tau_2$ is the
 product of the two, which is \eqref{eq.dzSurvival}.
...
\begin{remark}\label{remark.defectiveClock}
 ...
 which is the expected number of \emph{direct} offspring the current state still owes:
 integrating $\mathbf{1}^{\transpose}\excitation\decayedCounts$ forward from
 $\arrivalTimes[]_n$ gives $D/\decay$;
...
and it diverges.
The second holds by construction for the model of this chapter,
and the third whenever every type is reached --- when $\baseIntensity > 0$, for instance.
