...
$$\mathcal{B}_t := \big(P^a_t,\ P^b_t,\ \{(S^{a,i}_t, S^{b,i}_t) : i = 1, 2, \dots\}\big),$$
...
$S^{a,i}_t$ being the size at $P^a_t + (i-1)\tau$.

Everything in §3–§4 of the notation follows from $\mathcal{B}$ by definition:
the prevailing prices and the sizes resting at them, the spread, the mid-price, the imbalance.
...
What $\mathcal{B}$ drops is *whose* size traded.
It fixes how much is consumed at each price and at what price it prints,
because $\min(q', S(\pi))$ does not depend on how $S(\pi)$ decomposes into individual orders —
...
| --- | --- |
| what are the prices and sizes? how does the book evolve? | aggregate |
| how much size is ahead of *my* order? will it be filled? | order-level, id-indexed |
...
\qquad q_M = \min\Big(q,\ \sum_{i\ge 1} S^{b,i}_{t-}\mathbf{1}_{\{P^{b,i}_{t-}\,\ge\,p\}}\Big),$$
...
When it does, $N_s = +\infty$, that side empties, and its best price is undefined.
...
These carry the **new absolute size** at a price, not a signed change,
...
| tests | `tests/lob/test_aggregate_book.py::TestCaseB` (walks the book, residual below the old best bid, index shift, empty levels), `::TestExhaustedSide`, `::TestMarketOrders`; `tests/lob/test_delta_log.py::TestDeltaStorage` |
...
**5. A sell executed in full, with no remainder** &mdash; `sell 250 @ 999`.
...
**6. A sell that walks the book and rests the remainder** &mdash; `sell 400 @ 999`.
...
so each overrides `best_price` and the `set_size` that keeps its index in step.
The last one stops varying a single factor on purpose:
a real low-latency book *fuses* storage and index,
the sizes living in the tick-indexed array itself,
...
Two points belong to this package rather than to the theory.

**The reduction is derived, not quoted.** Dassios and Zhao state the scalar case with
i.i.d. jump sizes; ours are type-dependent, which the derivation permits because the
inter-arrival law depends on the state only through the current total intensity. It is
